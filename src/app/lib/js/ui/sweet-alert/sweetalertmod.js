  ...
  function SweetAlertMultiInputAmalgamate()
  {
    ...
    for(var i=0;i<deleteInputs.length;i++)
    {
      ...
      else if(inputElement.dataset.type=="team") {
        inputElement.setAttribute("placeholder","e.g. Lions");                
        inputBoxes.push(inputElement.value);        
      } 
      else if(inputElement.dataset.type=="image") {
        inputElement.setAttribute("placeholder","e.g. https://i.imgur.com/xVnx4R4.jpg");                
        inputBoxes.push(inputElement.value);        
      }            
      else if(inputElement.dataset.type=="color") {
        inputElement.setAttribute("ng-model","hexPicker.color");                
        inputElement.setAttribute("colorpicker", "hex");
        inputElement.setAttribute("color-parent", "true");
        inputElement.setAttribute("colorpicker-is-open", "isOpen");
        inputElement.setAttribute("ng-style", "{ 'background-color': hexPicker.color }");
        inputBoxes.push(inputElement.value);        
      }
      else if(inputElement.dataset.type=="password") {
