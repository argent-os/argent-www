(function() {
  ...
  angular.module('app.module.customers', [])
  ...
  .controller('CustomersController', ['$scope', '$state', 'notifications', '$rootScope', '$modal', '$log', 'UserFactory', '$http', 'DEFAULT_LOGO', 'LocalStorage', 'Currency', 'appconfig',
    function ($scope, $state, notifications, $rootScope, $modal, $log, UserFactory, $http, DEFAULT_LOGO, LocalStorage, Currency, config) {
...
(function() {
  ...
  function init(user, plans) {
      $http({
        url: 'https://api.stripe.com/v1/plans',
        headers: {
          "Authorization": "Bearer " + user.stripeToken
        },
        method: "GET"
      }).then(function (res) {
          $rootScope.plans = res.data.data;
      })        
    vm.user = user;
  ...
  UserFactory.getProfile().then(function (res) {
    init(res.data, $scope.getCustomers(res.data.stripeToken, 10));
  });
  ...
  $scope.limitSelected = 15;
   $scope.data = {
    ...
    $scope.getCustomers = function (token, limit) {
      ...
      $http({
        ...
        method: "GET",
        params: { limit: $scope.data.selectedOption.num }
      }).then(function (res) {
    ...
    //template urls are found within customers.html
    //be sure to make sure you are in customer.html or createcustomer.html for template url id
    $scope.openCreateCustomerModal = function (size) {
    ...
    $scope.openRecurringBillingModal = function (size, customer) {
      $scope.customer = customer;
      var modalInstance = $modal.open({
        templateUrl: 'rb.html',
        animation: false,
        controller: 'RbModalInstanceCtrl',
        windowClass: 'animated bounceIn',
        backdropClass: 'modal-backdrop',
        size: size,
        resolve: {
    ...
    $scope.deleteCustomer = function (customer, token) {
        $http({
          ...
          method: "DELETE"
            ...
            notifications.showError('Customer ' + res.data.id + ' deleted');  
    ...
    $scope.removeRow = function(name){        
        var index = -1;   
        var cusArr = eval( $scope.customers );
        for( var i = 0; i < cusArr.length; i++ ) {
          if( cusArr[i].name === name ) {
            index = i;
            break;
          }
        }
    ...
    $scope.openPlanDataModal = function (size, customer) {
      $scope.customer = customer;
      var modalInstance = $modal.open({
        templateUrl: 'pd.html',
        animation: false,
