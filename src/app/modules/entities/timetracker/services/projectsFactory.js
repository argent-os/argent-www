(function () {
    var projectsFactory = function ($firebaseArray, $firebaseObject, config, UserFactory, $rootScope) {
        ...
        factory.getProjects = function (user) {
            var ref = new Firebase(config.firebaseUrl + '/users/' + user.username + '/projects/');
            var list = $firebaseArray(ref);
            return list;
        };
    
        factory.getTasks = function (user) {
            var ref = new Firebase(config.firebaseUrl + '/users/' + user.username + '/tasks');      
            var list = $firebaseArray(ref);
            return list;
        };

        factory.getTaskById = function (user, id) {
            var ref = new Firebase(config.firebaseUrl + '/users/' + user.username + '/tasks/' + id);
        ...
        factory.getProjectById = function(user, id) {
            var ref = new Firebase(config.firebaseUrl + '/users/' + user.username + '/projects/' + id);
